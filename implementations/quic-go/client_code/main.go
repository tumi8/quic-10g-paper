...
var errUnsupported = errors.New("exit status 127")
...
func runTestcase(testcase string) error {
	...
	if testcase == "multihandshake" {
		return runMultiHandshakeTest(quicConf, urls)
	}
	...
	switch testcase {
	case "handshake", "transfer", "retry", "http3", "transportparameter", "goodput", "optimize", "qlog":
	case "chacha20":
...
func runMultiHandshakeTest(quicConfig *quic.Config, urls []string) error {
	for url := range urls {
		r := &http3.RoundTripper{
			TLSClientConfig: tlsConf,
			QuicConfig:      quicConfig,
		}
		err := downloadFile(r, urls[url], false)
		r.Close()
		if err != nil {
			return err
		}
...
func runVersionNegotiationTest(r *http3.RoundTripper, urls []string) error {
	if len(urls) != 1 {
...
func runMultiConnectTest(r *http3.RoundTripper, urls []string) error {
...
func runResumptionTest(r *http3.RoundTripper, urls []string, use0RTT bool) error {
	if len(urls) < 2 {
		return errors.New("expected at least 2 URLs")
	}
