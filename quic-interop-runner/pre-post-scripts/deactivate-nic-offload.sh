...
# Deactivate TCP Segmentation Offload (TSO)
# ethtool name: ""
ethtool -K ${INTERFACE} tso off

# Deactivate UDP Fragmentation Offload (UFO)
# ethtool name: "udp-fragmentation-offload" or "ufo"
# ethtool -K ${INTERFACE} ufo off # Seems not to exist, just ignore error

# Deactivate Generic Segmentation Offload (GSO), used by TCP and UDP
ethtool -K ${INTERFACE} gso off

# Deactivate Large Receive Offload (LRO), used by TCP
ethtool -K ${INTERFACE} lro off

# Deactivate Generic Receive Offload (GRO), used by TCP and UDP
